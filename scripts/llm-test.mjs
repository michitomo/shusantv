...
- fromはその行に実在する部分文字列。自信がなければ含めない(再現率より適合率を優先)。`,
  ...
  summary: `あなたは国会審議の自動字幕(誤変換あり)を、発言者ごとに要約するアシスタントです。
本文は書き換えません。発言者の交代でまとめ、各発言に「1行要旨」と「要約」を付けてJSONで返します。

入力は [index] 本文 の行が続きます。次のJSONだけを返してください(コードブロック不要):
{ "turns": [{"i": <その発言が始まるindex>, "sp": "<発言者名または役職>", "gist": "<25字以内の1行要旨>", "summary": "<2〜4文の要約>"}] }
規則:
- 発言者が交代するindexごとに1要素。同一発言者の連続発言は1つにまとめる。
- 発言者は本文中の呼称(例:委員長、○○大臣、○○くん)から推定。
- gistは見出し的に超簡潔に。summaryは論点・主張・答弁の骨子を中立的に。
